...
class DeformableDETR(nn.Module):
    ...
    def randshift(self, samples, targets):
        ...
        for i, (image, target) in enumerate(zip(samples.tensors, targets)):
            ...
            #target['boxes'] = target['boxes'].to(image.device)
    ...
    #def forward(self, samples_targets,unused_emb = None):
    def forward(self, samples_targets,yolo_bbox_query):
        fp16 = False
        ...
        if self.training:
            ...
            #print(self.nest2tensor(samples,tensor_type).shape)
            pre_samples, pre_targets = self.randshift(samples, targets)
            prepre_samples, _ = self.randshift(samples, targets)
            # 過去2フレーム
            #prepresamples2, _ = self.randshift(past_samples,targets)
            prepre_samples2 = None
            
            # ------yolo output -------
            #print(yolo_bbox_query)
            
            #pre frame pre_embed --> dict
            pre_out, pre_embed, _ = self.forward_once(pre_samples, prepre_samples, prepre_samples2 , yolo_bbox_query,pre_targets, targets)  
            ...
            if torch.randn(1).item() > 0.0:
                ...
                out, _ = self.forward_train(samples,yolo_bbox_query, pre_embed, past_samples)     
            else:
    ...
    @torch.no_grad()    
    def forward_once(self, samples: NestedTensor, train_samples: NestedTensor, past_samples:NestedTensor,yolo_bbox:torch.Tensor,targets=None,next_targets=None):
        ...
        #past_feat,_ = self.backbone(past_samples)
        ...
        for l, (feat, feat2) in enumerate(zip(features, pre_feat)):
            ...
            if l == 0:
                ...
                plt.savefig('w_input_yolov9/w_src_map_1024.png')
            ...
            #prepre_srcs.append(self.input_proj[l](src3))
        ...
        # yolo bboxを追加
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, memory = self.transformer(srcs, pre_srcs, masks, pos, yolo_bbox, query_embeds)
        outputs_classes = []
        outputs_coords = []
        #print('hs = ',hs.shape)
        ...
        #print(out)
        return out, pre_embed, _
    
    def forward_train(self, samples: NestedTensor,yolo_bbox:torch.tensor, pre_embed=None,com_samples=None):
        #fp16 = False
        #tensor_type = torch.cuda.HalfTensor if fp16 else torch.cuda.FloatTensor
        ...
        for l, (feat, feat2) in enumerate(zip(features, pre_feat)):
            ...
            #src shape =  torch.Size([4, 512, 96, 128])
            #src shape =  torch.Size([4, 1024, 48, 64])
            #src shape =  torch.Size([4, 2048, 24, 32])
        ...
        # print('train = ',yolo_bbox.shape)     
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, _ = self.transformer(srcs, pre_srcs, masks, pos, yolo_bbox,query_embeds, pre_reference, pre_tgt)           
