...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        #track
        """
        decoder_track_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points,
                                                          checkpoint_dec_ffn)
        """
    ...
    def forward(self, srcs, pre_srcs, masks, pos_embeds, detr_dict, query_embed=None, pre_reference=None, pre_tgt=None, memory=None):
        ...
        for lvl, (src, pre_src, mask, pos_embed) in enumerate(zip(srcs, pre_srcs, masks, pos_embeds)):
            ...
            if lvl == 0:
                ...
                plt.axis('tight')
                plt.axis('off')
                plt.savefig(save_path + '/w_input_{}.png'.format(list_num+1),bbox_inches='tight',pad_inches=0)
        ...
        if pre_reference is not None:
            ...
            #このtgtが追尾用の Ft Track Queryになってる    
            tgt = pre_tgt
            #print('pre tgt = ',tgt.shape)
            #[1,600,256]
            ...
            hs, inter_references = self.decoder_track(tgt, detr_flag, reference_points, memory,
                                                      spatial_shapes, valid_ratios, query_embed, mask_flatten)
            #hs.retain_grad()  # 中間テンソルの勾配を保持
            #print('Detect Decoder Norm = ',hs.grad)
            inter_references_out = inter_references
            #print(query_embed.shape)
        else:
            #print('------------ pre reference is None -------------')
            if self.two_stage:
                ...
                #print(pos_trans_out.shape)
                #[bs,500,512]
        
                """
                # detr referencepoint setting
                topk = 600
                topk_proposals = torch.topk(enc_outputs_class[..., 0], topk, dim=1)[1]
                topk_coords_unact = torch.gather(enc_outputs_coord_unact, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))
                topk_coords_unact = topk_coords_unact.detach()
                ...
                init_reference_out = reference_points
                #print('query embd = ',query_embed)
            query_embed = None
            #print('query emb = ',query_embed)
            #print('valid = ',valid_ratios)
            
            # Detect Decoder
            #print('Detect Decoder')
            
            #------ RTDETR の情報を埋め込む-------------
            #tgt_detr = detr_dict['tgt']
            #reference_points_detr = detr_dict['ref']
            #memory = detr_dict['mem']
            #spatial_shapes = detr_dict['space']
            #valid_ratios = detr_dict['valid']
            #mask_flatten = detr_dict['mask']
            #print(mask_flatten.shape)
            #print(reference_points.shape)
            
            #tgt = torch.cat([tgt,tgt_detr],dim=1)
            #reference_points = torch.cat([reference_points,reference_points_detr],dim=1)
            #print(tgt.shape)
            #print(reference_points.shape)
            #tgt_detr = detr_dict['tgt']
            tgt_old = tgt
            ...
            hs, inter_references = self.decoder(tgt_old, detr_dict, reference_points, memory,
                                                spatial_shapes, valid_ratios, query_embed, mask_flatten)
            #hs.retain_grad()  # 中間テンソルの勾配を保持
            #print('Detect Decoder Norm = ',hs.grad)
            #hs, inter_references = self.decoder(tgt, yolo_bbox_query, reference_points, memory,
                                                #spatial_shapes, valid_ratios, query_embed, mask_flatten)
            inter_references_out = inter_references
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.gate_q = nn.Sequential(
            ...
            nn.GELU(),
            #nn.utils.weight_norm(nn.Linear(d_model * 2, d_model)),  # Weight Normalizationを適用
        ...
        #self.fc_gate_1 = nn.Linear(d_model, 1)  # TransTrackクエリ用ゲート
        #self.fc_gate_2 = nn.Linear(d_model, 1)  # DETRクエリ用ゲート
    ...
    def forward(self, tgt, detr_dict ,query_pos, reference_points, src, src_spatial_shapes, src_padding_mask=None):
        # ---------- query learning module -----------
        
        #if tgt_new != None :
        #tgt_new = detr_dict['tgt']
        #new_pos = detr_dict['pos']
        if detr_dict is not None:
            #print('Detect tgt')
            tgt = detr_dict['output']
            print(tgt.shape)
        #else:
            #print('Track tgt')
            
        #tgt_q = torch.cat([tgt,tgt_new], dim=-1)
        #tgt_q = self.gate_q(tgt_q)
        #tgt = tgt_q
            
        """
        tgt_old = tgt
        combined_queries = torch.cat([tgt_old,tgt_new], dim=1)
        importance_scores = self.score_layer(combined_queries)  # shape: [1000, 1]
        importance_scores = importance_scores.squeeze(-1)  # shape: [1000]  
        top_scores, top_indices = torch.topk(importance_scores, k=500, dim=1,sorted=False)  # top_indices: [B, k]
        # 上位k個のクエリを取得
        B, N, D = combined_queries.size()
        batch_indices = torch.arange(B).unsqueeze(-1).expand(-1, 500)  # [B, k]
        tgt = combined_queries[batch_indices, top_indices]  # shape: [B, k, D]
        """
        #else:
            #print('tgt None')
        ...
        # cross attention
        #print('pad mask = ',src_padding_mask.shape)
        #print('src = ',src.shape)
        ...
        #print('tgt detr shape = ',tgt_detr.shape)
...
# track用のDecoder
"""
class DeformableTransformerTrackDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4,
                 checkpoint_ffn=False):
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, detr_dict,reference_points, src, src_spatial_shapes, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            #print('query =',query_pos)
            
            # Decoderの入力
            output = layer(output,detr_dict,query_pos, reference_points_input, src, src_spatial_shapes, src_padding_mask)
...
class DeformableTransformerTrackDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False):
        super().__init__()
        self.layers = _get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
