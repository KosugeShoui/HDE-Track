...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, past_samples: torch.Tensor,yolo_bbox:torch.Tensor,pre_embed=None):
        ...
        if pre_embed is not None:
            ...
            # STD Tracking Mode
            #print(query_embeds)
            hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, _ = self.transformer(srcs,pre_srcs,masks, pos,yolo_bbox, query_embeds, pre_reference, pre_tgt, memory)
            outputs_classes = []
            outputs_coords = []
            for lvl in range(hs.shape[0]):
                #print(lvl)
        ...
        # ここで検出と追尾の結果になっているっぽい
        #print(out['pred_boxes'],out['tracking_boxes'])
...
def build(args):
    if args.dataset_file == 'coco':
        ...
        num_classes = 3
    elif args.dataset_file == "visem_12":
        num_classes = 3
    else:
        num_classes = 3
